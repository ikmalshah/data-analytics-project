...
```{r}
data <- read.csv("~/R-project/data/VehicleFailure.csv")
```

```{r}
mycolor <- c("blue", "#FFC125", "darkgreen", "darkorange")
```
...
*This average labor cost was `r mean(data$lc)`.

*This average material cost was `r mean(data$mc)`.
...
Created by: KUPTM STUDENT - IKMAL SHAH
